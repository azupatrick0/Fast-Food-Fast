...
export class Orders extends Component {
  ...
  processCardPayment = async (token) => {
    ...
    const response = await Axios.post(`${process.env.BASE_URL_PROD}/api/v1/orders/checkout`, { token, totalAmount: getTotalAmount() });
    const mealOrdered = response  && await this.orderAMeal();
    mealOrdered && toast.success('Order made successfully');
